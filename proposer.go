...
func (p *Proposer) runTwoPhase() {
	...
	for p.decidedValue == nil {
		...
		case <-promiseFailedChan:
			logPrint("[proposer:%d] %v 小于当前accepter的N", p.getMe(), p.proposerID)
			continue
		case <-promiseSuccessChan:
			logPrint("[proposer:%d] %v 有可能会达到共识", p.getMe(), p.proposerID)//当前pomise已到半数
			// prepare success
		}
		...
		logPrint("[proposer:%d] phase 2, acceptMsg:%v(最后一个值是proposer计划提案值)", p.getMe(), acceptMsgReq)
		...
		select {
		// case <-time.After(200 * time.Millisecond):
		// 	// logPrint("[proposer:%d] 的提案未在规定时间内达到共识", p.getMe())
		// 	logPrint("超时")
		// 	continue
		case <-acceptFailedChan:
			logPrint("[proposer:%d] %v not reach consensuse", p.getMe(), p.proposerID)
			continue
		case <-acceptSuccesChan:
			p.decidedValue = acceptMsgReq.Value
			logPrint("[proposer:%d] %v reach consensuse Value: %v", p.getMe(), p.proposerID, acceptMsgReq.Value)
			return
		}
	}
}
...
func (p *Proposer) accept() AcceptMsg {
	...
	msg := AcceptMsg{
		...
		Value0: p.currentValue,//把proposer本身想提案的值也输出
	}
